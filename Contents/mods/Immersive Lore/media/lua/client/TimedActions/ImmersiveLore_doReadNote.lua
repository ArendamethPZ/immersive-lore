...
ImmersiveLore = ImmersiveLore or {}
ImmersiveLoreUtils = require("ImmersiveLore_Utils")
...
function ImmersiveLore_doReadNote:perform() -- Trigger when the action is complete
    ...
    if not ImmersiveLoreUtils.getSavedStory(self.item) then
        if isDebugEnabled() then
            print("NO MOD DATA FOUND. SETTING NEW.")
        end

        story = ImmersiveLoreUtils.generateNewStory(self.item, self.character)
    else
    ...
    ImmersiveLoreUtils.addStoryToSeenStories(self.character, story)

    ISBaseTimedAction.perform(self);
end

function ImmersiveLore_doReadNote:new(player, item) -- What to call in the code
    local o = {};
    setmetatable(o, self);
    self.__index = self;
    o.player = player
