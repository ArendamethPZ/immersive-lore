...
ImmersiveLoreTables = require("ImmersiveLore_Tables")
...
function ImmersiveLoreDebug.reRollAnyRoom(item, player)
	...
	local story = ImmersiveLoreUtils.getStory(nil, character)
...
function ImmersiveLoreDebug.reRoll(item, player)
	...
	local story = ImmersiveLoreUtils.getStory(savedRoom, character)
	item:getModData().ImmersiveLore_savedStory = story
...
function ImmersiveLoreDebug.getSpecificStory(item, player)
	local modal = ISTextBox:new(0, 0, 280, 180, "Enter a story ID", "0", nil, ImmersiveLoreDebug.getSpecificStoryClick,
		player,
