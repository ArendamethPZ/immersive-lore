...
function ISReadStoryUI:toggleCensor()
    ...
    if self:isCensorshipEnabled() then
        contentToShow = ImmersiveLoreUtils.censorContent(self.content)
    end
...
function ISReadStoryUI:createChildren()
    ...
    self.closeButton = ISButton:new(self:getWidth() - (btnWid * f) - 10, self:getHeight() - btnHgt - 10, btnWid * f,
        btnHgt, "Close", self, ISReadStoryUI.onOptionMouseDown)
    ...
    self.censorButton = ISButton:new(self.authorButton:getRight() + 10, self.authorButton.y, 30, 30, "#!%&!$*", self,
...
function ISReadStoryUI:prerender()
    ...
    if self.author and self:isAuthorVisiblityEnabled() then
        self:drawText("Author: " .. self.author, self.censorButton:getRight() + 10, self.authorButton.y, 1, 1, 1, 1,
            UIFont.NewMedium)
    end
